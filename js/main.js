  ...
  const PROJECTS = {
    ...
    golgol: {
      ...
      retrospective: '<p>해당 프로젝트의 배경 이미지와 기억의 조각은 AI를 활용해 제작했다. 기억의 조각은 큰 문제 없이 제작할 수 있었지만, 배경 이미지 제작 과정에서는 시행착오가 많았다.</p>' +
        ...
        '<p>주인공 "골골"의 에셋이 내가 구상했던 사이드뷰용이 아니라 탑뷰용 스프라이트 시트로 제작된 것이다. 처음에는 AI에게 "에셋 안의 스프라이트 시트를 잘라 애니메이션까지 적용된 캐릭터 데이터를 만들어 줘"와 같이 요청했지만, 계속해서 탑뷰 이미지 한 장만 참조되는 문제가 발생했다.</p>' +
